...
Read `HANDOFF_2026-09-09_S169.md` first. It is the full picture; this is the 30-second version.
...
1. **FIX THE FOG.** The owner ruled that a player's own RACE BACKDROP must be VISIBLE during BUILD;
   only the BUILDINGS, spawn and connectors hide. The shipped build fogs everything. Fix site
   `src/main.ts:628-629` — but NOT as a two-way swap, that was regression F1 (the 0.55-alpha
   backdrop then composites over every building). Needs a third layer.
