...
- **P2 (Standard) — GitHub Pages deploy + custom-domain swap path.** User-chosen domain: **`spark-online.space`** (purchased post-S16 by user; deploy plan accommodates the swap). Two-step landing:
  ...
  - **Step 2 (custom-domain swap, anytime after WHOIS verification clears):**
    1. **Pre-step (user, ~1 min):** click the ICANN WHOIS verification email Squarespace sent to daneshto@gmail.com after purchase (status next to domain says "Action Required" until done; suspension if not clicked within 15 days of purchase 2026-05-12).
