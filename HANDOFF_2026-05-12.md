...
- **BLOCKER — Cross-network play impossible: dev server is localhost only (user-reported post-handoff):** Vite at `localhost:31183` is loopback-only. Friend in another country has nowhere to load the SPARK app from. Trystero handles the actual P2P sync once both peers load the app, but the app-loading step is gated on a public URL. **S16 P2 (Standard): deploy to GitHub Pages.** Concrete plan: (a) `vite.config.ts` — add `base: '/the-spark/'` so asset paths resolve under the repo subpath; (b) new `.github/workflows/deploy.yml` running `npm ci && npm run build` on push to master, publishing `dist/` to `gh-pages` branch (use [peaceiris/actions-gh-pages@v3](https://github.com/peaceiris/actions-gh-pages) or similar); (c) GitHub repo Settings → Pages → enable for `gh-pages` branch; (d) verify `https://daneshto-dotcom.github.io/the-spark/` loads cleanly + Trystero WebRTC works under HTTPS (mandatory for getUserMedia / WebRTC in some browsers — GitHub Pages serves HTTPS by default, so this is fine). Quick same-day test alternative for validating P1 before P2 lands: `npx ngrok http 31183` exposes localhost to a temporary public URL.
...
- **P2 (Standard) — GitHub Pages deploy.** Concrete steps:
  1. Edit `vite.config.ts`: add `base: '/the-spark/'` (matches repo name).
  2. New `.github/workflows/deploy.yml`: triggers on `push: branches: [master]`; runs `npm ci && npm run build`; publishes `dist/` to `gh-pages` branch via `peaceiris/actions-gh-pages@v3` with `github_token: ${{ secrets.GITHUB_TOKEN }}` (no PAT setup needed; built-in token has Pages write permission).
  3. On GitHub: repo Settings → Pages → Source: "Deploy from a branch" → Branch: `gh-pages` / `(root)`.
  4. Verify public URL `https://daneshto-dotcom.github.io/the-spark/` loads cleanly post-deploy. Trystero needs no extra config — Nostr relays are public; WebRTC works under HTTPS (GitHub Pages = HTTPS).
  5. Send friend the URL. Friend opens it → clicks 1v1 → join with code. Real cross-country playtest unlocked.
