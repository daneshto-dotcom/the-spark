...
## OPEN ISSUES
- **NON-BLOCKING — § XV PRIME-AUDIT carry-forward (NEW S16):** `world.ts` grew 228 → 357 LOC (+129; over 280 trip-wire). Recommended S16 fix: extract S15 new dispatch handlers (START_GAME, END_TURN, RETURN_TO_TITLE, UPDATE_AVATAR_POS) + addScore helper to `src/state/gameMode.ts` (~80 LOC moved). Same Micro pattern as S14 P2.0 / S15 P1.
- **NON-BLOCKING — browser playtest not run post-S15 changes:** dev server was running pre-S15. With new TITLE screen on boot, user reload will see mode-select instead of immediate gameplay. All 291 unit tests pass + typecheck clean, but the actual cross-network 1v1 connection has not been exercised against a second peer (Trystero's Nostr relays + browser WebRTC stack).
- **NON-BLOCKING — Trystero bundle size:** +~40KB minified (LOCKED § 13.8 acknowledges). No build run yet.
- **CLOSED — S14 § XV PRIME-AUDIT carry-forward (controls.ts 565 LOC):** addressed by P1 extraction. controls.ts now at 479 LOC (post-P1) + ~62 LOC of P2 additions (Space handler + dispatcher injection + setPlayerId) = 542 LOC final, under 600 trip-wire.
...
**Immediate (Session 16 / playtest):**
1. **Restart dev server if not running**: `npx vite --port 31183 --strictPort`.
2. **User solo regression playtest**: pick "1 Player" → identical post-S14 gameplay (avatar pulse, multi-endpoint bonding, all 36 combos, scoreProgress, WIN at 50). Verify NO regression from the gameMode/currentPlayerId/scoreByPlayer plumbing.
3. **User cross-network 1v1 playtest** (top priority). Pre-S15 was hotseat-aspiration; S15 ships actual networked. Friend in different country can:
   - Open page → click "1v1 (2 Player)" → join pane → enter code shared by host → "Connect" → connection establishes ≤30s.
   - Host clicks "Begin Match" → both see same world → P1 places primitive → P2 sees it after ~RTT/2 + 100ms lerp → P2 presses SPACE → END_TURN → P2 places → loop until first to 50 → WIN screen in winner's color → POSTGAME → click returns to TITLE.
   - Kill one peer's tab to verify "CONNECTION LOST" overlay UX.
4. Tune feel constants if needed:
   - `NET_SNAPSHOT_HZ` (10) — raise to 15-20 if remote motion choppy at high RTT.
   - `NET_INTERPOLATION_MS` (100) — match snapshot interval; raising = smoother but laggier.
   - S14 carry-overs: AVATAR_PULSE_HZ, REDUNDANT_BOND_*. S13 carry-overs: cinematics constants.

**Short-term (post-playtest):**
5. § XV carry-forward (NEW S16): extract world.ts new dispatch cases + addScore to `src/state/gameMode.ts` (1 Micro priority, ~80 LOC moved; brings world.ts to ~280).
6. Per playtest signal: client-side AttractDrag prediction + reconciliation buffer (Grok R1 ask, ~150 LOC); delta-encoded NetSnapshot for bandwidth (Council R1 nice-to-have); host-migration stub if disconnects feel intrusive (Grok R2 ask).
7. Pick from `docs/phase-2-design-options.md` Tier 1+ matrix (Sever-as-disruption + Multi-color render is recommended next pair, ~220 LOC).

**Medium-term:**
8. **Phase 2 Tier-1+ implementation:** disruption suite (C/D/E) + multi-color rendering (F) + mega-combos (G).
9. Audio integration when Suno didgeridoo trance track lands.
