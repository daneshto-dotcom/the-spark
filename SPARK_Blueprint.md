...
**Companion documents:** [SPARK_v0.6_DESIGN.md](SPARK_v0.6_DESIGN.md) carries the diagnosis, the reasoning, and the S126–S150 roadmap. This document is the specification. Where they disagree, the design doc is newer.
...
This retires the G2 "family traits" backlog item — every pair does something, without designing 24 bespoke mechanics.
...
The castle is assembled out-of-match from the player's trophies (§ XIV). It is the player's identity object, the thing they defend, and the visible display of their history. It is not a blank start.
...
It also gives the 24 functional combos a purpose for the first time (§ V.3).

### VIII.4 — Energy Already Exists

`player.energy` accrues via `TICK_ENERGY` in the physics loop, rides the wire in the protocol allowlist, and renders as a gauge. v0.5 specified sinks that were never wired — disruption ended up gated by build-count instead. **Energy has accumulated forever and been spent on nothing.** v0.6 gives it its job.
...
> **Radius is linear; area is not.** The zone loses **~43% of its area**, not 25%. `FREE_SPARK_SOFT_CAP` must come down in step (~28–30) or free-spark density rises ~77% and the zone becomes an unreadable churn. See `SPARK_v0.6_DESIGN.md § 14`.

### X.2 — Spawn Trigger [LOCKED]

Per-tick base rate plus a bonus on player build events. Type distribution is **uniform random with no rarity tiers** — steering happens through directives (§ VII.3), not through the spawn table.
...
Snowball governors: the bank cap bounds economy scaling by build rate (§ VII.4), spawn rate scales with build events, and a distant castle pays a longer round trip.
...
Towers and defenders do **not** auto-retarget. The player clicks a tower and sets its priority — the Bloons TD interaction, with SPARK's categories:
...
The host emits a **full-world JSON snapshot at 10 Hz with no delta encoding** (~3 KB today).

Six players' worth of workers (~30 agents) roughly doubles that. Survivable on WebRTC, but it **scales linearly with every autonomous entity added.** Delta encoding is currently an optimisation; past a certain worker count it becomes a prerequisite.

### XV.5 — The Bundle [CONSTRAINT]

Main entry ≤ **750 KiB raw**, mechanically enforced by `scripts/check-bundle-size.mjs` as the last step of `npm run build`. Currently ~640 KiB — roughly **110 KiB of headroom for the entire v0.6 roadmap.**
...
Every new world entity must be deliberately wired into all four:

1. **Host migration** — castles, workers, banks, directives, upgrades and targeting must survive a handover.
2. **Save / load / replay** — the clear-rehydrate-advance-nextId pattern plus byte-identical replay coverage. *A persistent creature once rehydrated with `despawnAtTick = 0` and deleted itself instantly; workers carry the same shape of risk.*
3. **Teardown parity** — every teardown site, not the two that are easy to find.
4. **Disconnect / rejoin** — a rejoining player now restores a castle, a worker population and a bank, not just a cursor.
...
**Authority:** this document > `LOCKED_DECISIONS.md` > session-level tuning. Where `SPARK_v0.6_DESIGN.md` and this document disagree, the design doc is newer.
