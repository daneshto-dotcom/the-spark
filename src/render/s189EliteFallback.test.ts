...
describe('⛔ NEGATIVES — the fallback only ever fills a gap', () => {
  ...
  it('every other creature type has NO fallback (only the elite borrows a sheet on this branch)', () => {
    for (const t of Object.keys(CREATURE_CONFIGS) as CreatureType[]) {
      expect(atlasFallbackType(t), t).toBe(t === 't3PiranhaElite' ? 't3Piranha' : null);
    }
  });
