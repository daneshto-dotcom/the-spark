...
function struck(): World {
  ...
  // ⭐ S190 merge (wrath × render): `Player.raStrike` became `raStrikes` (≤ 3, cast order) in s188/wrath.
  if (w.players.get(P0)!.raStrikes.length === 0) throw new Error('fixture: the strike was refused');
  return w;
}

/**
 * ⭐ S190 merge (wrath × render) — seat 0 holds WRATH OF RA (`mummies.l10`: POWER OF RA at level 0, the
 * racial again at level 10) on wave 11, and casts `casts` strikes on the same tick at two points. The
 * per-charge draw loop must send EVERY strike through the above-units layer, not only the first.
 */
function struckWrath(casts: 1 | 2): World {
  const w = struck();
  const p = w.players.get(P0)!;
  p.draftPicks.splice(0, Infinity, 'racial', 'hp', 'racial');
  p.raStrikes = [];
  w.waveNumber = 11;
  dispatch(w, { type: 'CAST_POWER_OF_RA', playerId: P0, x: 700, y: 400 });
  if (casts === 2) dispatch(w, { type: 'CAST_POWER_OF_RA', playerId: P0, x: 1200, y: 500 });
  if (p.raStrikes.length !== casts) throw new Error(`fixture: expected ${casts} strikes, got ${p.raStrikes.length}`);
  return w;
}
...
describe('⭐⭐ S190 R190-H — the owner\'s strike art lands in the layer ABOVE the unit sprites', () => {
  ...
  it('⭐ WRATH OF RA — a SECOND charge in the air draws above the units too (both strikes, not only the first)', () => {
    setRaStrikeArtForTests(shippedArt());
    const count = (casts: 1 | 2): { above: number; ground: number } => {
      const w = struckWrath(casts);
      w.tick += RA_COLUMN_TICKS + 2; // column 0 of every strike has just landed
      const { r, ground, above } = mount();
      r.sync(w);
      return { above: textures(above), ground: textures(ground) };
    };
    const one = count(1);
    const two = count(2);
