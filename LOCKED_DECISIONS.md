...
> **Why these two strikes happened here (V6-0.3 / S131).** The fog fork was ruled on a re-verified CI fact rather than a vote: **no CI job runs `vitest` or `typecheck`** (zero hits across `.github/workflows/`; `deploy.yml`'s only verification is `npm run build`). So `constants.lock.test.ts` is a LOCAL-only guard, which makes this stale prose the only defence a future session actually reads — and leaving it armed would instruct that session to re-arm a regression the user has now locked twice. Items §2/5 and §2/6, plus the worker-vision clause, are **NOT** ratified here and are bound to V6-1.1/V6-1.2; touch no radius (`SPAWNER_RADIUS` is still 250 and item 4 says re-judge after the V6-1.2 shrink).
