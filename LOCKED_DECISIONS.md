...
| **§ 7** | Godly recipes | **AMENDED S140 P1 (owner ruling).** The component sizes are load-bearing for the economy: pentagram 5 · lightningHub 6 · Helga 7 · **laserTurret 7 (was 8 — retuned in S140 P1 so both TOWER recipes fit CASTLE_BANK_CAP 7)** · Voltkin 8, each an EXACT isolated component. **Never tune the bank cap without this table in view.** ⚠ **CORRECTION, S140 P1:** this row previously read "a bank cap ≥ 5 makes direct assembly rational and deletes the carve-down tactic (B4)". That threshold was WRONG — a compressed misreading of B4. Both `BACKLOG.md` B4 and `src/constants.ts` state the danger condition as a bank **≥ the biggest recipe**, and B4's harm is a CONJUNCTION (hard type filter ∧ buffer ≥ biggest recipe) whose first conjunct S134 ruled out of the game entirely ("AN ORDERED BUILD QUEUE, RRTS-STYLE — *NOT* A FILTER"). The carve-down the owner actually plays — "build big, erase to 9 of a type" (`sudokuEvent.ts:36`) — is gated at the NONET's 9 and survives the S140 raise untouched. Corrected rather than retired: the clause is real, its threshold was mis-stated. |
...
### Bundle charter — main entry ≤ 1100 KiB raw (S95 P1; 560→750 S101; 750→900 S145; 900→1000 S180, 2026-09-16; 1000→1100 S188, 2026-09-23)

The main entry chunk (`dist/assets/index-*.js`) must stay **≤ 1100 KiB raw** (= 1,126,400 bytes; raw byte size / 1024, NOT gzip) — the figure enforced by `scripts/check-bundle-size.mjs` (`CAP_KIB = 1100`). ⚠ S162: this sentence still read **750** while the heading above it, the S145 paragraph below it and the enforcing script all said 900, because the S145 raise was appended as new prose without revising the clause's own MUST sentence — so the one normative line in the charter was the only place that disagreed with the gate. History follows. History: a "550 KiB" soft cap was raised from 500 in S57 but lived only in handoff prose — no `LOCKED_DECISIONS` entry, no CI guard — so it silently crept to 553.0 KiB by S94. S95 set it to **560 KiB** AND made it **mechanically enforced**: `scripts/check-bundle-size.mjs` runs as the last step of `npm run build`, so `deploy.yml` CI (`npm ci && npm run build`) fails on a regression. **S101 raises it to 750 KiB (owner directive).** Rationale: S100's tower-defense slice put the entry at 570.9 KiB — 10.9 KiB over — which **hard-failed the deploy build and silently kept the feature off the live site for a whole session** while it sat "done" in git. The cap is self-imposed, not a platform limit; the gzipped transfer is ~185 KiB (fine for a WebGL game), so the gate's failure mode (block deploy) was far costlier than the few-KiB-gzip it guarded. 750 gives real headroom for the rest of TD Phase 1 while keeping the mechanical tripwire against *unbounded* creep. A future breach is a **deploy-blocker to fix before marking a priority done** — NOT a "soft budget" to wave past (that mislabel was S100's root failure). Heavy/optional UI (codex, bots, debug, NONET overlay, quickmatch) still stays **lazy code-split**; big art/audio goes to `public/` assets, never the bundle. To change the cap: bump `CAP_KIB` in the script AND this clause together.

**S188 raises it 1000 → 1100 KiB, IN ITS OWN COMMIT, the S180 way.** The six S188 racial branches merged put
the entry at 943.0 KiB and the script's warning band fired (*"only 57.0 KiB headroom — RAISE THE CHARTER NOW"*)
with four more owner-requested branches still to land (the footer-arrow layering, THE SWARM, WRATH OF RA and
the Ra strike art). Same reasoning as S180 below: a self-imposed cap whose failure mode is a stale live site
must be raised before the feature that needs the room, never debugged against it.

**S180 raises it 900 → 1000 KiB, IN ITS OWN COMMIT, before the feature that needed the room.**
The character sheet put the entry at 843.8 KiB and the script's own warning band fired: *"only 56.2
KiB headroom — RAISE THE CHARTER NOW before it hard-fails a deploy."* Two things make this a
deliberate raise rather than goalpost-moving. (1) Both Council seats called raising the cap **inside**
the feature commit constraint-evasion — GEMINI: *"masking the cost of a feature by moving the
goalposts invalidates the purpose of having a budget"* — so the raise lands first, alone, and the
feature's true cost (9.7 KiB) is visible in its own diff. (2) The charter is self-imposed and its
failure mode is the expensive one: S101's root failure was a 10.9 KiB breach hard-failing the deploy
and keeping a finished feature off the live site for a whole session. The gzipped transfer remains
well inside what a WebGL game spends. Heavy/optional UI stays lazy code-split; art and audio stay in
`public/`, never the bundle.
...
| 7 | Memory-fog curve | ~~Linear opacity 1.0→0.4, desat 0→0.7 over 30s [PHASE 2]~~ **STRUCK — see §14** | **DO NOT RE-ARM.** V6-0.3 (S131) re-ratification: this row's `desat 0→0.7` and `opacity →0.4` describe a *tinted, partially-desaturated* fog. §14 records the user deciding TWICE (S63, then again at the S86 round-6 playtest — "the stupid blue fog is back") that fog is **pure black**, and `constants.lock.test.ts` now asserts `MEMORY_FOG_COLOR === 0x000000`. This row is the exact stale prose that caused the S85→S86 regression: S85 P4b restored a dim tier from an old design note. Kept visible rather than deleted so the trap is legible, but it is **superseded by §14**. |

~~Phase 1 = solo, no fog.~~ **STRUCK (V6-0.3 / S131) — see §14.** That sentence is false twice over: fog ships in Phase 1 (`MEMORY_FOG_COLOR` is live and locked), and Phase 1 is not solo-only (1v1 and VS-BOTS both ship). Read §14 for the fog decision of record. Items 4–6 remain placeholders — verify against `constants.ts` before citing, as item 4 shows this block goes stale. Spawner/canvas/spawn-rate must be locked.

> **Why these two strikes happened here (V6-0.3 / S131).** The fog fork was ruled on a re-verified CI fact rather than a vote: **no CI job ran `vitest`**, so `constants.lock.test.ts` was a LOCAL-only guard — which made this stale prose the only defence a future session actually reads, and leaving it armed would instruct that session to re-arm a regression the user has now locked twice. Items §2/5 and §2/6, plus the worker-vision clause, are **NOT** ratified here and are bound to V6-1.1/V6-1.2; touch no radius (`SPAWNER_RADIUS` is still 250 and item 4 says re-judge after the V6-1.2 shrink).
>
> ⚠ **CORRECTION + UPGRADE (S131 P2).** The claim as first written here — and as carried by several handoffs — was *"no CI job runs `vitest` **or `typecheck`**"*. The `typecheck` half is **false**, and the error is the grep: `package.json` defines `build` as `tsc -b && vite build && node scripts/check-bundle-size.mjs`, so `npm run build` has always run `tsc -b`. Types (and the bundle cap) have been CI-gated all along, transitively; only `vitest` was missing. **That gap is now closed** — `deploy.yml` gained a gating `npx vitest run` step in S131 P2, so `constants.lock.test.ts` is, for the first time, a **CI-blocking** guard rather than a local tripwire. That is precisely what the S86 Council asked for when it wrote *"Docs alone didn't stop it; a CI-blocking test does"* — the assertion at the top of `constants.lock.test.ts` now genuinely blocks a master-push deploy. The strikes above stand on their own merit regardless, and the prose is still what a session reads first.
...
| `world.effects` BOND_SEVERED | serialized w/ `cause` union | **SEVEN members** (corrected V6-0.3/S131 — this row said four for ~30 sessions): `'player' \| 'physics' \| 'godly' \| 'creature' \| 'bomb' \| 'chewer' \| 'drone'`. `'bomb'` S71 P1, `'chewer'` S102 #2, `'drone'` S113 Batch C. `'godly'` is **dead-but-kept-for-back-compat** post-S27 (S34 P2-18 DROP-not-delete pattern) and is in this EFFECT union but **NOT** in the `SEVER_BOND` action union (`world.ts`) — so never write an exhaustive `switch` over `cause`. |
| BOND_SEVERED `actor?: PlayerId` / `victim?: PlayerId` | additive-optional | V6-0.3 (S131) sever attribution — **NO `PROTOCOL_VERSION` bump and NO `SCHEMA_VERSION` bump**, on the `creatureId?` precedent below and the bump bar beneath it. Three verified grounds: `deserializeEffect` reconstructs by NAME (not positional/packed), `parseNetMessage` never descends into `snapshot.effects`, and `ARC_FLASH.creatureId?` is the identical shipped precedent. **TWO** fields, not one: an actor id alone cannot answer "am I the victim?" — by emit time the bond and (on the delete path) its endpoints are already gone. `victim` = `primA.placedBy` for all seven causes; `actor` is absent for `'physics'` (that dispatch passes a hardcoded `asPlayerId(0)` sentinel) and `'godly'` (unreachable). ⚠ **Reach in networked play is ~1/6**, not 100% — see the BOND_SEVERED delivery note in `severToastRenderer.ts`. |
