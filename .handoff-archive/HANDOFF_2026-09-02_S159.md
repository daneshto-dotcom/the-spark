...
- ⚠ **P9 is a large uplift to feel:** 9 drones a fight instead of 3.
  ⛔ **CORRECTED BY S160 P2(b) — the dial order here was BACKWARDS.** This said *"`DRONE_MAX_PER_SPAWNER`
  (3) is the first dial, the 5 s cadence the second."* Measured against the shipped constants the cap
  does **nothing**: occupancy at every due slot is `ceil(480/300) − 1` = 1, so 3 → 2 leaves the count
  at 9, and peak live is **2** against a ceiling of 3. Only 3 → 1 bites, and bluntly (5/fight). **The
  cadence is the only linear lever** — count = 2700/interval, so 450 t → 6, 600 t → 4. **Correct order
  for "too strong": `DRONE_EMIT_INTERVAL_TICKS`, then `DRONE_MAX_CONNECTORS`, then the cap.** Both the
  9 and the cap's slack are now asserted in `lightningHubDelivers.test.ts`.
