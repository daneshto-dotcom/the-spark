...
**P4 — Scope Amendment urgent BLOCKER fix: pin Nostr relays (Micro, commit `12de8cd`).** Post-handoff playtest BLOCKER: user + brother both stuck at "connecting" in 1v1 lobby across separate networks. A.0 via node_modules inspection: silent npm bump `trystero ^0.20 → ^0.24` since S15 P2 wiring + 0.24's `@trystero-p2p/nostr` module picks 5 random relays from 55 defaults via `shuffle(defaults, strToNum(config.appId))`. Shuffle IS deterministic per appId (both peers land on same 5) but the default list includes many personal / dead / geo-flaky endpoints (basspistol.org, chorus.almostmachines.dev, etc); both peers picked the same dead set → no Nostr signaling → no WebRTC offer delivery → stuck. Fix: pin 6 known-reliable public Nostr relays in `src/net/transport.ts` NOSTR_RELAYS const, pass via `relayConfig.urls` + `redundancy = NOSTR_RELAYS.length` so ALL 6 are used (no sub-sampling): relay.damus.io / nos.lol / relay.mostr.pub / purplerelay.com / relay.nostr.band / nostr.wine. Both peers now deterministically connect to the same 6 reliable relays; first to deliver WebRTC offer wins, rest are redundancy. LOCKED §13.1 updated: Trystero pin `^0.20` → `^0.24` (matches reality) + NEW NOTE block codifies the 6-relay set + future-bump audit protocol (run `npm install trystero@latest` + 1v1 playtest before locking — package layout changed materially between 0.20 and 0.24 with the move to `@trystero-p2p/*` scoped sub-packages). Typecheck exit 0. Build 394.74 KB bundle (+0.18 KB net from constants). Network-layer change, no unit tests (manual playtest verifies on live URL).
