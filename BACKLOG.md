...
| **Sim-worker default-on** | ✅ **PASSED** — owner playtested `?worker=1`, reported smooth. Flip the default and drop the flag gate in **V6-1.1**. Note the evidence was a phone browser; if a weak laptop later misbehaves, that configuration is what went untested. **6 literals across 4 files hard-code the flag** (`e2e/worker.spec.ts:32`, `worker-bots.spec.ts:35`, `worker-duel.spec.ts:69,70`, `worker-heap.spec.ts:433,468`) — corrected in S137 A.0 from "5 files", which both miscounted the files and omitted `worker-heap.spec.ts:468`. Also verified: `src/dev/probeHarness.ts:339-345` REFUSES TO ARM while the flag is set, so after a default-on flip the probe harness becomes refuse-**by-default** and that guard must be inverted in the same change. |
