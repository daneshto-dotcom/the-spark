**STATUS: COMPLETED** — all three items shipped in cc4e382 and verified live (index-vSzv8e82.js).
Keeps moved to KEEP_RING_RADIUS 420; STRUCTURE_GROW_IMPULSE deleted (visual flash kept, MERGE_IMPULSE
left alone); bots scoped to their own porch with a new PULL goal on the shipped PULL_FROM_BANK intent.
PROTOCOL_VERSION 16->17. tsc 0 / vitest 2148/2148 / e2e:gating 32/0 / deploy 4/4.
⚠ TWO CONSEQUENCES CARRIED, both needing an owner playtest: vs-bots is now EASIER (bots gated on
gatherer throughput + the 1.97x longer haul), and the opening economy is SLOWER until CASTLE_BANK_CAP
rises to 12-13 (the ruled counterweight, deliberately not in this change).
