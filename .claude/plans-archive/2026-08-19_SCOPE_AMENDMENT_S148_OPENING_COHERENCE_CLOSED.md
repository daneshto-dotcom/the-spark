**STATUS: CLOSED, PARTLY SUPERSEDED** (stamped S159 P4) — PA (empty opening) and PB (per-kind weapon damage) shipped as S148 P2 and were deploy-verified. Of PC, **two of its three parts shipped** in later sessions: **castle HP** (`CASTLE_MAX_HP = 1500`) and **elimination** (`damage.ts:116` — *"already fallen — idempotent, never double-fires the win"* — plus the fallen/survivors win gate at `gameState.ts:76`).
⛔ **THE ONE PART THAT NEVER SHIPPED, CARRIED FORWARD RATHER THAN CLOSED: CASTLE GUNS.** A grep for `castleGun|keepGun|castleTurret` returns nothing in `src/`. It is not un-designed — it is specced with an owner ruling already attached: `SPARK_TD_SESSION_SPECS.md:59` Q4, *"castle attacks any enemy units that attack it"* ⇒ retaliation-only against anything that damaged this castle within 300 ticks (the window itself is CLAUDE-overridable), and `:208-230` costs the work as a reuse of the defender FSM shape. It is also the same ground as the owner's later **elemental keeps** idea (a per-seat element per keep, all shooting at short range defensively). This document is closed because PC is no longer where that work lives; the work is real and open, and S159's handoff carries it as the top recommendation. Do not read this stamp as "castle guns were dropped" — read it as "castle guns need their own owner-approved PDR".
