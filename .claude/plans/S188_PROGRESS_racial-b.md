...
|---|---|---|---|
| THE RISEN | `zombies.l0` | in progress | `state/racial/theRisen.ts` (new), `creatures/creatureLifecycle.ts` death arm, `damage.ts` (one line: attacker threaded into `damageCreature`), `raceUnitEmit.ts` (additive: `spawnRaceUnitAtCastle`) |
