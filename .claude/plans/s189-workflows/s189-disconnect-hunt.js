export const meta = {
  ...
  phases: [
    { title: 'Hunt', detail: 'six independent lanes: transport, wire, exceptions, load (+C5 lag), coverage+history, quickmatch seat (C6)' },
...
THE REPORT (owner, 2026-09-24, with a screenshot): in a live 2-player multiplayer match on deploy #1 (commit 5c6615f, PROTOCOL_VERSION 50 — the S188 racial-upgrades release) he got the overlay "CONNECTION LOST" / "peer dropped — return to title to retry". It is NOT known whether both tabs were freshly loaded onto the new build. His playtest with his brother the same day adds, verbatim: C4 "connection was lost at like wave five or something … Need to make sure connection is established and always on"; C5 "it was lagging at about wave five. I thought we fixed the lags"; C6 "Quick match … I'm player one … my brother connects … it switches me to player two puts him as player one … only happens to this workstation … if my brother goes in as player one and then I join him then I'm player two".

READ THE CODE HERE, read-only: MAIN = ${MAIN} — branch master @ 441c832, whose src/ is byte-identical to the LIVE deploy #2 (15035b9). Deploy #1 (5c6615f) differs from it ONLY by the racial-a/racial-d fix rounds (git -C "${MAIN}" diff --stat 5c6615f 15035b9 -- src : 24 files, NONE under src/net and not src/main.ts), so master is the right tree for both builds. Use absolute paths. Do NOT read source from ${MAIN}\\.claude\\worktrees\\* — those are other, unmerged branches (four of them are being edited right now).
...
const LANES = [
  ...
  { key: 'quickmatch', prompt: `LANE F — QUICKMATCH SEAT (C6). This lane's symptom is NOT the overlay: set produces_this_exact_overlay=false on every finding and read "reachable_in_live_2p_match" as "reachable in a real 2-player quickmatch on the production build". The owner's expectation: whoever was in the quickmatch lobby FIRST stays player one (seat 0). Read src/net/quickmatch.ts in full and every production caller (main.ts quickmatch / lobby / seat-assignment paths). ESTABLISHED (S189, by hand): seekers join the SMALLEST advertised code; a PEERLESS host that hears a smaller code DEMOTES and joins it; a host with peers never demotes — so the smaller code, not arrival order, decides who hosts. Find: (1) how the advertised code is generated (clock, crypto.getRandomValues, Math.random, localStorage, a stored identity / key fingerprint, anything per-machine) and whether a per-machine input biases it so ONE workstation's code is systematically LARGER — it would then always lose the smaller-code contest and always demote, which is exactly "only happens to this workstation" (the discriminating fact: when the brother hosts first and the owner joins, the owner correctly becomes player two); (2) timing and reach: can the first arrival hear the second's advertisement and demote even though it arrived first; does beacon / relay reach differ per machine (relay set selection, Nostr relay list, TURN); (3) where seat numbers are assigned (is the host always seat 0?) and whether anything other than who-hosts decides seats; (4) the fix shape for "first in the lobby stays player one", deterministic on both sides in both arrival orders, with a total-order tie-break — what each seeker must advertise (e.g. a lobby-entry timestamp/epoch) and the cost (does it touch the wire, a HELLO/LOBBY_* field, and so the protocol?). Read the existing tests (src/net/*quickmatch*.test.ts) and say whether any covers two seekers in both arrival orders.` },
]
