...
- master (= deploy #1, LIVE, PROTOCOL_VERSION 50): ${MAIN}  (branch master @ 8c369c4)
...
const BRANCHES = [
  ...
  },
  {
    ...
    lenses: [
      { key: 'sim', text: `LENS 1 — SIM, WIRE, DETERMINISM. Audit every sim-side change: racialPerks registry (RACIAL_PERK_REQUIRES, perkDraftIndex generalisation — does it change the index/offer for ANY existing L0/L5 perk or for a seat of another race?), draftEvent offer/deadline/validation with the new picks parameter (an unoffered pick must still be refused — the P1 fix), raChargesFor / raCastsInWave / raChargesLeft, the reducer (prune earlier-wave strikes, append; NaN/Infinity/off-canvas no-op per amendment A1; cast only during FIGHT; benched/eliminated), strikes overlapping in one wave, the charge-indexed pattern seed (determinism: total order, no Map order, no float), the four sites for raStrikes (factory, reset between matches, serialize/deserialize with validation and a CAP on array length from the wire, hash union + projection + contribution test, worker mirror), host migration (a promoted successor inherits raStrikes), bots casting Ra (src/bots/botRa.ts — deterministic? host-only? uses the same reducer path?), and the sever path now calling applySeverBond directly (does that bypass anything besides the actor gate that SHOULD still apply — ownership of the target bond, the S157 own-shapes exemption, BOND_SEVERED cause, effects/toasts?). And the protocol: raStrike -> raStrikes means a deploy-#1 v50 peer and this build cannot interoperate — confirm and state the bump owed. SANDWORM must not be reachable in any form.` },
      { key: 'ui-merge', text: `LENS 2 — UI REACH, BRIEF, AND MERGE ONTO MASTER. (a) The footer skill slot: 46 px square Sprite + overlay Graphics + pips; the S182 fill enumeration went 8 -> 9 — is every new opaque fill hit-tested or declared decorative, and does the click target match the drawn square in expanded AND collapsed footer states? Texture lazy-load and fallback (a missing /art/upgrade-cards/l10-mummies.webp — ra-vfx ships it, this branch does not); refused/disabled states SAY WHY. (b) Brief compliance against the owner's words above: 3 casts per FIGHT, offered only with mummies.l0, the icon from the card art, bots cast it. (c) Run git -C "${MAIN}" merge-tree --write-tree master s188/wrath; list conflicts; read master's versions of every touched file (master is ~75 commits ahead of 3b63c92: the cards branch rewrote draftOverlay.ts, the castle branch added castle-panel plates, racial-c's own code on master may have been changed by later merges or deploy-#1 fixes). Find drift: master-side code that still reads Player.raStrike (singular) and would not compile or would silently read undefined after the merge; tests on master that pin raStrike; the 16-card test; RACIAL_PERK_BUILT blocks. Give the exact resolution per conflict.` },
    ],
  },
  {
    ...
    lenses: [
      ...
      { key: 'art-merge', text: `LENS 2 — ART, UI, BRIEF, AND MERGE ONTO MASTER. (a) The atlas t3-vampires-bat-swarm (fly/attack/die), the build-scattered-sheet-atlas.mjs change (claimed byte-identical for elite piranha + corpse eater — check the claim's method), the renderer fallback to the bat sheet at 2x when the swarm sheet is missing, the l10-vampires.webp card on the level-10 tile (master's draft overlay draws which tiles for which wave? is a level-10 tile even reachable on master — draft index 2, wave 11?). (b) Brief compliance. (c) Run git -C "${MAIN}" merge-tree --write-tree master s188/swarm; list conflicts; read master's versions of every touched file (master is ~72 commits ahead of 4b52fdd, including the racial-d fix round only on s188/deploy2-candidate — check whether swarm touches any file that deploy2-candidate also changes: git -C "${MAIN}" diff --stat master s188/deploy2-candidate). The progress file lists "merge-owner reconciliation owed" items — verify each and give exact resolutions (build-upgrade-cards.py, the 16-card test, RACIAL_PERK_BUILT blocks, canon notes).` },
    ],
  },
  {
...
const JOBS = BRANCHES.flatMap((b) => b.lenses.map((l) => ({ b, l })))
