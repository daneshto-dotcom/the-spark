...
**Why Step 1 + 1.5 + Step 2 in same session** (Council R1 Grok #6 mitigation):
The CODE commits land in one session — vite.config base flip, public/ files,
LOCKED §13.9 amendment. The Custom-Domain TOGGLE in Pages Settings is
user-async after `dig` confirms DNS resolves; same-session DNS propagation is
NOT required (commits push, user toggles when DNS catches up — typically
15min to a few hours).
