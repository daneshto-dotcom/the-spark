...
- Fix (goblinRenderer.ts only): `export function atlasFallbackType(type)` → `'t3Piranha'` for
  ...
  anchor (elite cellW 226 vs 200 — width only). ⚠ S190 merge: the "portrait NOT given the fallback"
  line that stood here went stale — s188/swarm's SWARM-B1 made `portraitTexture` use `atlasFallbackType`
  too, so on the merged tree the card's portrait borrows the base sheet exactly as the board does.
