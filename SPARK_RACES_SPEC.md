...
| **R108 → SUPERSEDED BY R148 (S168)** | ~~**THE RACE TOWER IS TIER 3 AND IT IS FED, goblin-tower style.**~~ ⛔ **The owner reversed this in S168** — see R148. The original text is kept because the tower's FEED path is unchanged and still shipping; only the claim that feeding is its ONLY output is dead. Original: **THE RACE TOWER IS TIER 3 AND IT IS FED, goblin-tower style.** A new tier BELOW the current floor — *"we don't have tier three. we start from tier four."* It produces **the same unit the castle emits**, on demand, by being fed. Owner: *"that's easy to implement"* — and it is, because `goblinTowerFeed.ts` is the working precedent. |
...
| **R148** | ⭐ **THE TIER-3 RACE TOWER ALSO PRODUCES FREE, ON THE CASTLE'S OWN CLOCK.** Owner, S168: *"the tier 3 tower does not produce or spawn creatures! it should produce spawn at similar rate as the castle does"*. **This SUPERSEDES R108's fed-only reading.** Implemented as `RACE_UNIT_EMIT_INTERVAL_TICKS` — literally the castle's constant (R107, ~30 s), so the two can never drift apart. ⚠ **FEEDING IS UNCHANGED**: the tower is now BOTH a free trickle and an on-demand unit per shape fed. He asked to add the trickle, not to remove what he had. ⭐ Corroboration that this was always the intent — the shipped tier-3 sprite sheets carry a `spawning` row that no code could draw (`T3_TOWER_STATE_ROWS` skips row 1).
